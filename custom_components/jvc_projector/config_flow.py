...
from .const import CONF_MODEL, DOMAIN, NAME
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                ...
                mac, model = await get_device_info(host, port, password)
            except InvalidHost:
...
async def get_device_info(
    ...
    """Get device info for config flow."""
    device = JvcProjector(host, port=port, password=password)
    try:
        await device.connect()
        return await device.get(cmd.MacAddress), device.model
    finally:
